...
public ResponseEntity<?> addRegistration(
    ...
    if (registrationRepository
        ...
        return ResponseEntity.badRequest()
                .body("Student is already registered for this sub-event");
    }

    // Check capacity

    long registrationCount =
            registrationRepository.countBySubEventId(
                    registration.getSubEventId());

    if (registrationCount >= subEvent.getCapacity()) {

        return ResponseEntity.badRequest()
                .body("This sub-event is already full");
    }
